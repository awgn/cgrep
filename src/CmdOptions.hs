...
options = cmdArgsMode $ Options
          ...
          ,     language_force = Nothing    &= help "Force the language" &= explicit &= name "force-language"
          ,     language_map = False        &= help "Lists the language mappings" &= explicit &= name "language-list"
          ...
          ,     jobs = Nothing              &= groupname "\nConcurrency" &= help "Number threads to run in parallel" &= explicit &= name "threads" &= name "j"
